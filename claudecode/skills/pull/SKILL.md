...
1. **始终使用 rebase，绝不 merge**：所有 pull 操作使用 `git pull --rebase`，保证 commit 呈一条直线，禁止使用会产生分叉 merge commit 的方式
...
2. 如果 stash pop 出现冲突：
   - 这些是本地暂存变更与最新拉取代码的冲突
   - 同样执行冲突处理流程（第四步的逻辑）
   - ⚠️ **注意**：pop 出现冲突时 stash **不会**自动删除，必须进入第六步手动清理本次 stash

3. 如果 stash pop 成功，确认 stash 已自动清除：
   ```bash
   git stash list
   ```
   stash pop 成功会自动 drop 本次 stash，无需额外操作。但为保险，仍进入第六步确认。

### 第六步：清理本次暂存

第五步恢复 stash 之后，确保本次 pull 过程中创建的临时 stash 被妥善清理，避免 stash 栈堆积。

1. 委派 developer 查看 stash 栈：
   ```bash
   git stash list
   ```

2. 识别并清理本次创建的 stash（消息以 `auto-stash-before-pull-` 开头）：
   - **stash pop 成功**：stash 已自动删除，确认无 `auto-stash-before-pull-` 残留即可
   - **stash pop 有冲突（已解决）**：stash 仍在栈中，需手动清理：
     ```bash
     # 先确认是本次 stash 再 drop
...
3. **安全约束**：
   - 只清理本次 pull 创建的临时 stash（`auto-stash-before-pull-` 前缀）
   - 绝不清理其他历史 stash，避免丢失用户其他暂存工作
   - drop 前再次确认 stash 消息，避免误删
...
- 当前工作区状态
- stash 是否已完全恢复
- 本次临时 stash 是否已清理（`auto-stash-before-pull-` 应无残留）
- 是否存在遗留的冲突文件
...
git config pull.rebase true     # 全局默认 rebase
```
...
| `git stash drop`              | 丢失暂存         | ⚠️ 仅清理本次 `auto-stash-before-pull-` stash，drop 前确认消息 |
| `git rebase --abort`          | 取消 rebase      | ✅ 安全，回到操作前状态   |
| `git push --force`            | 重写远程历史     | ❌ 禁止使用              |
| `git merge`（会产生 merge commit）| 分叉历史，非线性 | ❌ 禁止使用（rebase 替代） |
...
- 本次 pull 创建的临时 stash（`auto-stash-before-pull-` 前缀）按第六步清理
- 绝不自动 drop 其他历史 stash，由用户决定
...
2. 可自动解决的简单冲突
3. stash pop 无冲突的恢复
4. 清理本次 `auto-stash-before-pull-` stash（收尾操作，仅限本次创建）
