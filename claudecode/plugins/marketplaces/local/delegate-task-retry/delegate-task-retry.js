...
/**
 * Delegate Task Retry for Claude Code
 *
 * 功能：监听 Task 工具执行结果，检测常见 delegate/task 错误模式，通过 stderr 发出重试指导
 *
 * Claude Code Hook: PostToolUse -> Task
 *
...
process.stdin.on("end", () => {
  ...
  const guidance = detectAndBuildGuidance(output);
  if (guidance) {
...
function detectAndBuildGuidance(output) {
  ...
  let guidance = `[task CALL FAILED - IMMEDIATE RETRY REQUIRED]\n\n**Error Type**: ${errorInfo.errorType}\n**Fix**: ${pattern.fixHint}`;
