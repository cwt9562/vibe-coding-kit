...
2. 分析变更语义，规划提交分组，排除敏感文件
3. 展示提交计划（文件列表 + 提交信息），询问用户确认
4. `git commit -m "..."` — 执行提交
5. `git log --oneline -5` — 确认结果

**多仓库**（当前目录非 git 仓库）：
1. 扫描子目录，对每个仓库查看暂存变更
2. 跨仓库整体分析，对齐术语，汇总提交计划，一次确认
3. 逐仓库执行：`git -C <repo> commit -m "..."`
4. `git -C <repo> log --oneline -3` — 逐仓库确认结果
