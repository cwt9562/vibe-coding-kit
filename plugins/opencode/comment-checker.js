/**
 * Comment Checker Plugin for OpenCode
 *
 * 功能：检测 write/edit/multiedit 工具写入代码中的 AI 风格注释
 * 支持语言：Java / JS / TS / Vue / Shell
 * 依赖：纯 JS 正则，无外部依赖
 *
 * 检测的 AI 风格注释：
 * - 解释性废话（"This function adds one to one"）
 * - 变更记录型（"changed from X to Y", "refactored", "moved from to"）
 * - TODO/未完成标记
 * - 被注释掉的代码
 *
 * 放行的模式：
 * - BDD 关键词（given, when, then, arrange, act, assert）
 * - Linter 指令（noqa, @ts-ignore, eslint-disable, type: ignore 等）
 * - Shebang（#!/bin/bash）
 */

// ============================================================
// 第 1 部分：正则模式（从 go-claude-code-comment-checker 翻译）
// ============================================================

/**
 * AI 风格注释正则模式列表
 * 按 omo comment-checker 的 agent_memo.go 翻译
 */
const AI_MEMO_PATTERNS = [
  ...
  [/^[\s#/*-]*=\s*\w+\s*\(/i],                          // `// =foo()`
  [/^[\s#/*-]*\(\)\s*=>/i],                             // `// () =>`
  [/^[\s#/*-]*(if|else|for|while|switch)\s*\(/i],       // `// if (...)`
  [/^[\s#/*-]*try\s*\{/i],                              // `// try {`
  [/^[\s#/*-]*catch\s*\(/i],                            // `// catch (`
  [/^[\s#/*-]*return\s+\w/i],                           // `// return foo`
  [/^[\s#/*-]*import\s+\w/i],                           // `// import Foo`
  [/^[\s#/*-]*export\s+(default|const|function|class)/i],
  [/^[\s#/*-]*@?[a-z]+\s*\(\s*['"][^'"]+['"]\s*\)/i], // `// foo('bar')` 或 `// @Component(...)`
  [/^[\s#/*-]*\{[\s\S]*?\}\s*;?\s*$/],                  // `// { foo: bar }`
  [/^[\s#/*-]*<\w+[^>]*>/i],                            // `// <Component>`
  [/^[\s#/*-]*class\s+\w+/i],                            // `// class Foo`
  [/^[\s#/*-]*function\s+\w+/i],                        // `// function foo`
  [/^[\s#/*-]*const\s+\w+/i],                            // `// const foo`
  [/^[\s#/*-]*let\s+\w+/i],                             // `// let foo`
  [/^[\s#/*-]*var\s+\w+/i],                             // `// var foo`
...
/**
 * 判断是否应该放行（BDD / directive / shebang）
 */
  ...
  if (isShell) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (trimmed.startsWith("#") && !trimmed.startsWith("#!")) {
        comments.push({ text: line, lineNumber: idx + 1, type: "line" });
      }
    });
  } else if (isVue) {
    // Vue: 混合 HTML / JS / CSS 注释
    // 1. HTML 注释 <!-- -->
    const htmlCommentRegex = /<!--[\s\S]*?-->/g;
    let match;
    while ((match = htmlCommentRegex.exec(content)) !== null) {
      const startLine = content.substring(0, match.index).split("\n").length;
      comments.push({ text: match[0], lineNumber: startLine, type: "block" });
    }
    // 2. JS/TS 行注释和块注释（在 <script> 标签内，但简化处理：全局扫描）
    extractJSComments(content, lines, comments);
    // 3. CSS 块注释（<style> 标签内，简化为全局 /* */）
    extractCSSBlockComments(content, lines, comments);
  } else if (isJS) {
    extractJSComments(content, lines, comments);
  } else if (isJava) {
...
function extractJSComments(content, lines, comments) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const idx = content.indexOf(lines[i], i === 0 ? 0 : content.indexOf(lines[i - 1]) + lines[i - 1].length);
    const trimmed = lines[i].trim();
    if (trimmed.startsWith("//")) {
      comments.push({ text: lines[i], lineNumber: i + 1, type: "line" });
    }
  }
...
/**
 * 提取 CSS 块注释（Vue <style> 或纯 CSS 中的 /* */）
 */
function extractCSSBlockComments(content, lines, comments) {
  const blockRegex = /\/\*[\s\S]*?\*\//g;
  let match;
  while ((match = blockRegex.exec(content)) !== null) {
    const startLine = content.substring(0, match.index).split("\n").length;
...
export const CommentCheckerPlugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      ...
      pendingCalls.set(input.callID, { filePath, content, tool });
    },
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (tool === "apply_patch" || tool === "applypatch") {
        ...
        if (metadata?.files && Array.isArray(metadata.files)) {
          for (const f of metadata.files) {
            ...
            const filePath = f.movePath ?? f.filePath;
            const content = (f.after ?? "").trim() || (f.before ?? "").trim();
            if (!content) continue;
            const bad = detectAIComments(content, filePath);
            if (bad.length > 0) {
              output.output += buildWarningMessage(bad);
            }
          }
        }
        return;
      ...
      const { filePath, content } = pendingCall;
